...
function chkPass(pwd) {
	...
	/* Modify overall score value based on usage vs requirements */
		...
		if (nNumber > 0 && nNumber < nLength) {	
			nScore = parseInt(nScore + ((nNumber-nConsecNumber) * nMultNumber));
			sNumber = "+ " + parseInt((nNumber-nConsecNumber) * nMultNumber);
		}
		if (nSymbol > 0) {	
			nScore = parseInt(nScore + ((nSymbol-nConsecSymbol) * nMultSymbol));
			sSymbol = "+ " + parseInt((nSymbol-nConsecSymbol) * nMultSymbol);
		}
		if (nMidChar > 0) {	
		...
		for (var c=0; c < arrCharsLen; c++) {
			var oImg = $e('div_' + arrCharsIds[c]);
			var oBonus = $e(arrCharsIds[c] + 'Bonus');
...
function togPwdMask() {
	var oPwd = $e("passwordPwd");
	var oTxt = $e("passwordTxt");
	var oMask = $e("mask");
	if (oMask.checked) { 
...
function initPwdChk(restart) {
	...
	/*$("passwordPwd").value = "";*/
	/*$("passwordTxt").value = "";*/
	$e("scorebar").style.backgroundPosition = "0";
	if (restart) {
